...
args.logs_dir += args.u_func_name + '_' + str(round(args.hyperparameter,2)).replace('.', '-') + "_repl_" + str(args.repl_n) +'/'
...
convex_IB = ConvexIB(n_x = n_x, n_y = n_y, problem_type = problem_type, network_type = network_type, K = args.K, beta = args.beta, logvar_t = args.logvar_t, logvar_kde = args.logvar_kde, train_logvar_t = args.train_logvar_t, u_func_name = args.u_func_name, hyperparameter = args.hyperparameter, dataset_name = dataset_name,method = args.method,repl_n=args.repl_n)
